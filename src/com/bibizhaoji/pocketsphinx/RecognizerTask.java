...
public class RecognizerTask implements Runnable {
	...
	class AudioTask implements Runnable {
		...
		void init(LinkedBlockingQueue<short[]> q, int block_size) {
			...
			this.rec = new AudioRecord(MediaRecorder.AudioSource.DEFAULT, 8000, AudioFormat.CHANNEL_IN_MONO, AudioFormat.ENCODING_PCM_16BIT, 8192);
		}
	...
	public RecognizerTask(Context context) {
		pocketsphinx.setLogfile(Environment.getExternalStorageDirectory().getPath() + "/Android/data/zuoshu/pocketsphinx.log");
		Config c = new Config();
		/*
					...
					if (hyp != null) {
						...
						if (hypstr != partial_hyp) {
							Log.d(getClass().getName(),
									"Hypothesis: " + hyp.getHypstr() + " Uttid: " + hyp.getUttid() + " Best_score: " + hyp.getBest_score());
